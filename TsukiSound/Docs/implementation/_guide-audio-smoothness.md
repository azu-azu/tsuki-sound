...
## What is Smoothness? — 滑らかさとは

> 滑らかさとは、音量・周波数・波形が
> *連続的に変化している* と耳が認識する状態のこと。
> 音は「連続」が崩れるとノイズになる。
...
実装中にすべてを気にできないときは、この3つだけは守る：

1. **attack = 0 を絶対に使わない** — 必ずクリック音が発生する
2. **cutoff / Q を急に動かさない** — フィルタ発振の原因
3. **ゼロクロスを必ず揃える** — 開始・終了点でのパチパチ音を防止
...
| **ジジジ音（クリックレス/エイリアシング）** | エンベロープの不連続性 | 低音200ms+、中音80ms+に延長 |
...
デチューン・レイヤーや複数音の重ね合わせで、「ジジジ」という附帯音が発生することがある。

**原因**:
1. **クリックレスノイズ**: 音の開始/終了点での波形の不連続性
2. **折り返し雑音 (Aliasing)**: エンベロープの急激な変化が高次倍音を発生させる

### 解決策: エンベロープの平滑化

低音域ほど不連続性に敏感なため、attackを長めに設定する。

```swift
// ノイズ抑制のためのattack設定
let melodyAttack: Float = 0.15   // 150ms（維持）
let bassAttack: Float = 0.20     // 200ms（低音は長めに）
let chordAttack: Float = 0.08    // 80ms（最低60ms以上）
```
...
| 周波数帯域 | 推奨attack | 理由 |
|-----------|-----------|------|
| 低音（200Hz以下） | 200ms+ | 不連続性を最も敏感に知覚する |
| 中音（200-500Hz） | 80ms+ | バランス重視 |
| 高音（500Hz以上） | 60ms+ | 鋭さを抑えつつクリア感維持 |

### なぜ低音ほど長いattackが必要か

音響心理学的に、人間の耳は低周波の不連続性を**より敏感に**知覚する。
低音の波形は1周期が長いため、急激な立ち上がりが「クリック」として明確に聞こえる。

```
100Hz = 1周期 10ms → attack 200ms で約20周期分確保
440Hz = 1周期 2.3ms → attack 80ms で約35周期分確保
```
...
| Date | Version | Change |
|------|---------|--------|
| 2025-11-30 | 1.4 | 「ジジジ」ノイズ対策セクション追加（エンベロープ平滑化、周波数帯域別attack推奨値） |
