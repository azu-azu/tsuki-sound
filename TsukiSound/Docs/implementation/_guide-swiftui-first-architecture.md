...
**TsukiSound は SwiftUI-First アプリケーション。UIKit API は原則使用しない。**
...
```bash
# UIKit import を検索
grep -r "import UIKit" TsukiSound/

# UIKit 型を検索
grep -r "UIColor\|UIFont\|UIView\|UILabel" TsukiSound/
```
...
**TsukiSound は SwiftUI-First。UIKit は過去のもの。新しい時代の設計で進める。**
