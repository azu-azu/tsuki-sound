...
**Phase 1**: 音の生成（AudioService基盤）
**Phase 2**: 音の安全性（Quiet Break、Safe Volume、Fade）
**Phase 3**: 音の統合（システム全体への拡張）

### 学びのポイント

1. **Live Activityは"見えない設定"が多い**
   - Info.plistの1行で動作が変わる
   - `public` 修飾子の有無でビルドエラー
   - デフォルト値の設定が初回体験を左右

2. **音源管理は"状態のクリア"が重要**
   - `stop()` だけでは不十分（配列に残る）
   - `clearSources()` で明示的な削除が必要
   - エンジンの起動順序がノード接続に影響

3. **Dynamic Islandは"情報の取捨選択"が鍵**
   - コンパクト表示は極小（アイコン2つまで）
   - テキストよりアイコンが効果的
   - 展開表示でも情報は最小限に

4. **Now Playingは"OSとの対話"**
   - リモートコマンドはユーザーが期待する動作を
   - メタデータは視認性を重視
   - 再生状態の同期が体験の質を決める
...
### パフォーマンス
- TrackPlayerの複数同時再生（ミキシング）
- 大容量音源ファイルのストリーミング再生
- メモリ使用量の最適化

### 音質
- 高品質音源の追加（48kHz/24bit対応）
- リバーブ・エコーエフェクト
- 3Dオーディオ（Spatial Audio）
