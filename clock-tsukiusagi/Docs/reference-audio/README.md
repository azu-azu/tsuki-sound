...
This directory contains field recordings and other external audio files that are **only** used for analysis, preset tuning, or reproduction experiments. These files are *not* bundled with the shipping application and should stay outside `clock-tsukiusagi/Resources/Audio`.
...
1. Run the analyzer to pull quick metrics:
   ```bash
   python scripts/analyze_sea_and_seagull.py clock-tsukiusagi/Docs/reference-audio/sea-and-seagull-wave-5932.mp3
   ```
