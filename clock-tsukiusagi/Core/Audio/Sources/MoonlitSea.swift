...
/// 深夜の海音源
/// ピンクノイズとゆっくりしたLFOで深海の呼吸を表現します
public final class MoonlitSea: AudioSource {
    // MARK: - Properties
    ...
    ///   - lfoFrequency: LFO周波数（深海の呼吸周期）
    ///   - lfoMinimum: LFO最小値
    ///   - lfoMaximum: LFO最大値
    public init(
        noiseAmplitude: Float = 0.4,
        lfoFrequency: Double = 0.25,
        lfoMinimum: Double = 0.03,
        lfoMaximum: Double = 0.10
    ) {
        ...
        _sourceNode = AVAudioSourceNode { _, _, frameCount, audioBufferList -> OSStatus in
            ...
            for frame in 0..<Int(frameCount) {
                ...
                // Calculate LFO (slow breathing pattern)
